...
function DISPLAY_EVENTS(top, left) { return 'jQuery("#eventFinder_popup").remove();    jQuery("<div>"+eventFinder_events+"</div>").attr("id","eventFinder_popup").css({top:"'+top+'px", left:"'+left+'px"}).prependTo("body");   jQuery("<div>close</div>").attr("id","eventFinder_close").prependTo("#eventFinder_popup");jQuery("#eventFinder_close").click(function(){jQuery("#eventFinder_popup").remove(); return false;})';}
var CLEAR_EVENTS = 'var eventFinder_events="";';
var DISPLAY_JQUERY_EVENTS = 'jQuery(".eventFinder_selected").each(function(){var dEvents=$(this).data("events");if(!dEvents){return;}for(var type in dEvents){jQuery.each(dEvents[type],function(key,handlerObj){eventFinder_events+="<p><b>jquery."+type+"</b> - "+(jQuery().jquery.indexOf("1.4")!==-1?handlerObj.handler:handlerObj)+"</p>";})}});';
...
jQuery('.eventFinder_domEvent, .eventFinder_jqEvent, .eventFinder_jqLive').mousedown(function(event) {
